...
def check_authorized_access(case_dict: CaseDict):
    """
    越权测试专用的处理数据生成器。
...
def get_app_api_path(path: Path) -> list[CaseDict]:
    """返回接口测试用例。
    Args:
    ...
    """
    case_dict = api_test_data(path)[0]
...
def get_pc_api_path(path: Path) -> list[CaseDict]:
    """返回接口测试用例。
    Args:
        path(Path): 用例文件地址，模板见/data/template/接口测试用例.xlsx
    Returns:
        list: 一个CaseDict列表，每个元素为一条用例.
    """
    case_dict = api_test_data(path)[1]
    sql_result = DB_CONN.get_asdict("""
    SELECT
        '越权测试' as api_name,
        (select p2.perm_name from permission p2 where p2.perm_id=p.up_perm_id) as father_perm_name,
...
@pytest.mark.parametrize("case_dict",get_app_api_path(Path(TEST_DATA_DIR,  'gateway_service', '接口测试用例_only_app_authorized.xlsx')))
def test_app_normal(app_perm_api: Manage, case_dict: CaseDict):
    app_perm_api.test(case_dict, eval(case_dict['处理数据'] or 'None'))

@pytest.mark.parametrize("case_dict",get_pc_api_path(Path(TEST_DATA_DIR,  'gateway_service', '接口测试用例_only_app_authorized.xlsx')))
def test_pc_normal(app_perm_api: Manage, case_dict: CaseDict):
    app_perm_api.test(case_dict, eval(case_dict['处理数据'] or 'None'))