...
@pytest.mark.parametrize("case_dict",get_pc_api_path(Path(TEST_DATA_DIR,  'gateway_service', '接口测试用例_only_pc_authorized.xlsx')))
def test_pc_normal(pc_perm_api: Manage, case_dict: CaseDict):
    pc_perm_api.test(case_dict, eval(case_dict['处理数据'] or 'None'))

@pytest.mark.parametrize("case_dict",get_app_api_path(Path(TEST_DATA_DIR,  'gateway_service', '接口测试用例_only_pc_authorized.xlsx')))
def test_app_normal(pc_perm_api: Manage, case_dict: CaseDict):
    pc_perm_api.test(case_dict, eval(case_dict['处理数据'] or 'None'))